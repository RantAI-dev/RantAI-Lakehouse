...
## Addendum 3: a user-created connector's credential is named after the connector
...
Addendum 2 left exactly one kind of connector able to hold a working
credential: the ones a migration seeds. `reject_allowlisted_secret_ref`
refuses, on `POST /api/connectors`, every ref that matches
`CONNECTOR_ALLOWED_SECRET_REF_PATTERNS`, and both resolvers — the API's
`AllowlistedSecretResolver` and Dagster's `secret_resolver.resolve_secret_ref`
— resolve only refs that match those patterns. `0023`'s header says it
plainly: "only migration-seeded connectors satisfy both". A connector
created through the console could never pass "Test", never ingest, and
`PUT /api/connectors/{id}/secret` could never succeed for any connector.

The reason for the refusal still stands: a `connector:manage` principal
chooses the `host`, so letting them name any allowlisted credential lets
them send another connector's credential to a host they control.

### Decision

**A user-created connector may only name credentials derived from its own
...
The id is generated by the server at creation, so the client does not name
the ref. It chooses a source (`env` or `file`) and a kind for each slot;
the server builds the ref and returns the names the operator must
provision. Rotation (`PUT .../secret`) accepts only a name derived from
the connector's own id.

Why this cannot reach someone else's credential:

- Derived names always begin `CONNECTOR_CONN_` / `connector_conn_`,
  because every generated id begins `conn-`. The seeded refs
  (`CONNECTOR_PG_*`, `CONNECTOR_S3_*`) do not, so no user connector can
  derive one. A test fails the build if a seeded ref ever does.
- Ids contain only `[a-z0-9-]`, so mapping `-` to `_` is one-to-one. No
  allowed suffix is an `_`-separated tail of another, so two different ids
  cannot derive the same name.
- The resolvers are unchanged: every derived name already matches the
  allowlist patterns. The binding is enforced where refs are written, the
  only place a caller chooses one.

### Consequences

- The operator provisions a credential per connector
  (`CONNECTOR_CONN_..._PASSWORD` in the API's and Dagster's environment,
  or a mounted file) after the connector is created. Until then "Test"
  reports the ref as unresolvable, honestly.
- An `env:` credential needs a restart of the processes that read it; a
