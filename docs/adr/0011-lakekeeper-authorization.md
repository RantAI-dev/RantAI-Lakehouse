...
| `lakehouse-api-reader` | `describe` | WS2 amendment A0 (added 2026-09-12, after `lakehouse-api` grew read-only `/api/lakehouse/*` catalog routes): read-only metadata access — list namespaces, list tables, load table metadata — never table data, never a write. `describe` was tried first, per the amendment's instruction, and measured sufficient for all three calls against this Lakekeeper (v0.13.3; proof: `docker compose -p ws2-a0 up`, torn down after), so `select` (which also permits reading table data) was never granted. Own principal, not a reuse of `gold-export` (`create`/`modify`/`select` — too broad for a read-only surface) or `clickhouse-reader` (holds `modify`, and is another service's identity). |
