...
- **Agent/tool execution is real, but bounded to the copilot's own registered tools.** Digital-employee/agent definitions, runs, and approvals are real (Postgres); interactive chat and headless `POST /api/agents/employees/{id}/run` both execute a fixed registry of 47 tools (alerts, connectors, pipelines, saved queries, governance, ops, Gold export, Bronze maintenance) through the same permission-checked, audited dispatch path — every write is either inline-confirmed or routed to a human-approval queue (`docs/adr/0012-copilot-tool-governance.md`). This is not a general autonomous-agent runtime: it cannot call arbitrary code or tools outside the registry, and there is still no document retrieval (see the RAG/vector entry above).
