...
// constraint still permits) — the filter offers those legacy values too,
// so its own value type must widen the same way.
export const PIPELINE_KIND_OPTIONS: { value: PipelineKind | string; label: string }[] = [
 ...
 * millis>` (`lakehouse_store::pipelines`'s id generator); a Dagster job id
 * is never prefixed `pl-`. Derived here rather than trusting a field that
 * does not exist on the wire.
 */
function pipelineOrigin(pipeline: Pipeline): "orchestrator" | "authored" {
...
export function getPipelineColumns({
  ...
  return [
    ...
    {
      id: "origin",
      accessorFn: (pipeline) => pipelineOrigin(pipeline),
      header: ({ column }) => (
          ...
          <TooltipTrigger
            render={
              <Badge variant="outline" className="text-xs font-normal">
                {pipelineOrigin(row.original) === "orchestrator" ? "Orchestrator" : "Authored"}
              </Badge>
            }
          />
          <TooltipContent>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span
          ...
          title={row.original.source ?? undefined}
        >
          {/* Null for a Dagster job: no per-job lineage exists until an
              op-graph reader ships. */}
          {row.original.source ?? "—"}
        </span>
      ),
      enableColumnFilter: true,
