...
export function PipelinesPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {state.status === "success" && state.data.orchestratorError ? (
        // Said out loud: the list is short because half of it could not be
        ...
        <p className="rounded-md bg-amber-500/5 px-3 py-2 text-xs text-amber-600 dark:text-amber-400">
          Orchestrator jobs are not listed — the orchestrator could not be
          reached ({state.data.orchestratorError}). Pipelines authored in the
