...
export function getEmployeeColumns(): ColumnDef<DigitalEmployee>[] {
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const r = row.original
        // budgetSpent/budgetReserved are unmeasured until spend-per-tool-call
        // tracking exists — show a dash rather than a false 0/percent.
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="tabular-nums">
          {fmtMeasured(row.original.successRate, formatPercent)}
        </span>
      ),
      enableColumnFilter: true,
      meta: {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="tabular-nums">
          {fmtMeasured(row.original.approvalRate, formatPercent)}
        </span>
      ),
      enableColumnFilter: true,
      meta: {
