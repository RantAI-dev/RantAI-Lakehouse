...
export interface Pagination<T> {
  ...
  /**
   * `false` when a tenant-scoped caller refused this endpoint
   * (`routes::catalog::query`'s `catalog_tenant_refusal` path) — the page
   * comes back empty (`items: []`, all counts `0`) rather than throwing,
   * so the UI must render `reason`, never an empty "no data" state.
   */
  supported?: boolean;
  reason?: string;
}
