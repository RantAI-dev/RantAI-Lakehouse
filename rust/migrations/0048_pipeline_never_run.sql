...
--
-- `last_run_at` defaulted to now() and `freshness_lag_seconds` to 0
-- (`0007_pipelines.sql`), so a pipeline created a moment ago reported
-- "Last run 1m ago · Fresh · 0s" before anything had executed it. Both
-- become nullable, and the default goes away: unknown is written as NULL
-- and read as "Never".
...
-- Rows authored before this migration carry the same fabricated "ran just
-- now" stamp; clear it for the ones that have no orchestrator job to have
-- run them.
