...
-- The two seeded saved queries pointed at tables that do not exist in this
-- lakehouse (`gold.revenue`, `hot.customers`, `lake.orders_history`), so
-- the only two examples a new user could open both failed on Run. Point
-- them at marts that are actually served.
UPDATE saved_query
