...
/// the same way [`to_render_spec`] renders a built-in `&'static` one:
/// everything except `sql`, plus its origin. There is no shared
/// `to_render_spec` overload across the two `ChartSpec` types (one
...
/// hand.
fn render_stored_spec(spec: &store::ChartSpec, source: ChartSource) -> Value {
    let mut map = Map::new();
...
/// `runSpec` in `dashboard/route.ts`: an empty `sql` (text tiles) needs no
/// query; any other failure is captured per-tile rather than failing the
/// whole response.
async fn run_spec_sql(ch: &ChClient, id: &str, sql: &str) -> (String, Value) {
    if sql.is_empty() {
...
/// grouped into a mart → column-set map.
async fn mart_columns(
    ch: &ChClient,
) -> Result<HashMap<String, HashSet<String>>, lakehouse_clickhouse::ChError> {
    let rows = ch
        .rows(
            "SELECT table, name FROM system.columns WHERE database='serving'",
            None,
